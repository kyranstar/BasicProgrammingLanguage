...
public class BigSurdVec implements Comparable<BigSurdVec> {
     ...
     * The value of zero.
     */
    public static final BigSurdVec ZERO = new BigSurdVec();
    
    /**
     * The value of one.
     */
    public static final BigSurdVec ONE = new BigSurdVec(BigSurd.ONE);
    ...
    protected final void normalize() {
        ...
        final Vector<BigSurd> newter = new Vector<BigSurd>();
    ...
    public int signum() {
        ...
        if (terms.size() == 3) {
            ...
            lhs = lhs.sqr();
            final BigSurd rhs = new BigSurd(terms.elementAt(offsig).sqr(),
                    Rational.ONE);
    ...
    public BigDecimal bigDecimalValue(final MathContext mc) {
        ...
        for (int addpr = 1;; addpr += 3) {
            final MathContext locmc = new MathContext(
                    mc.getPrecision() + addpr, mc.getRoundingMode());
            res[1] = BigDecimal.ZERO;
            for (final BigSurd j : terms) {
                res[1] = BigDecimalMath.addRound(res[1],
                        j.bigDecimalValue(locmc));
            }
            if (addpr > 1) {
    ...
    public BigSurdVec sqr() {
        ...
        for (int i = 0; i < terms.size(); i++) {
            resul.terms
                    .add(new BigSurd(terms.elementAt(i).sqr(), Rational.ONE));
        }
        for (int i = 0; i < terms.size() - 1; i++) {
            for (int j = i + 1; j < terms.size(); j++) {
                resul.terms.add(terms.elementAt(i).multiply(terms.elementAt(j))
                        .multiply(2));
            }
        }
        resul.normalize();
