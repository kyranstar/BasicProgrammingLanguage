...
public class BigDecimalMath {
     ...
     * @since 2009-08-13
     */
    static public BigDecimal gamma(final MathContext mc) {
        /* look it up if possible */
    ...
    static public BigDecimal sqrt(final BigDecimal x, final MathContext mc) {
        ...
        final MathContext locmc = new MathContext(mc.getPrecision() + 2,
                mc.getRoundingMode());
    ...
    static public BigDecimal hypot(final BigDecimal x, final BigDecimal y) {
         ...
         */
        final BigDecimal zerr = x.abs().multiply(x.ulp())
    ...
    static public BigDecimal exp(final BigDecimal x) {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            if (Math.pow(xDbl, TAYLOR_NTERM) < TAYLOR_NTERM
                 ...
                 */
                final MathContext mcTay = new MathContext(err2prec(1., xUlpDbl
                        / TAYLOR_NTERM));
                 ...
                 */
                final MathContext mc = new MathContext(expxby10.precision()
                        - exSc);
                /*
                ...
                while (exSc > 0) {
                    ...
                    exSc -= exsub;
                    final MathContext mctmp = new MathContext(
    ...
    static public BigDecimal log(final BigDecimal x) {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            BigDecimal zpown = z;
            final double eps = 0.5 * x.ulp().doubleValue()
             ...
             */
            final MathContext mc = new MathContext(err2prec(
                    resul.doubleValue(), xUlpDbl / xDbl));
            return resul.round(mc);
        }
    } /* BigDecimalMath.log */
     ...
     * @since 2009-08-08
     */
    static public BigDecimal log(final int n, final MathContext mc) {
        /*
         * the value is undefined if x is negative.
         */
        if (n <= 0) {
    ...
    static public BigDecimal log(final Rational r, final MathContext mc) {
        ...
        if (r.compareTo(Rational.ZERO) <= 0) {
             ...
             * the result, epsr/r.
             */
            final double eps = prec2err(Math.log(r.doubleValue()),
                    mc.getPrecision());
    ...
    static public BigDecimal pow(final BigDecimal x, final BigDecimal y) {
        if (x.compareTo(BigDecimal.ZERO) < 0) {
             ...
             * |log(x)*err(y)|+|y*err(x)/x|
             */
            final double errR = Math.abs(logx.doubleValue()
                    * y.ulp().doubleValue() / 2.)
    ...
    static public BigDecimal powRound(final BigDecimal x, final Rational q) {
         ...
         */
            ...
            if (q.isIntegerFrac()) {
                 ...
                 */
                final BigDecimal reserr = new BigDecimal(0.5
                        * q.abs().doubleValue()
                ...
                for (;;) {
                    ...
                    final BigDecimal err = res.multiply(reserr,
                     ...
                     */
                    if (eps.divide(res, MathContext.DECIMAL64).abs()
                            .compareTo(reserr) < 0) {
                        /*
                 ...
                 */
                final int precq = 3 + err2prec(x.ulp()
                        .divide(x, MathContext.DECIMAL64).doubleValue()
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            if (res.compareTo(p) > 0) {
                ...
                if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
                     ...
                     */
                    final int k = (int) (res.precision() / Math.log10(1.0 / res
                            .doubleValue())) / 2;
                    final MathContext mcTay = new MathContext(err2prec(
                    ...
                    for (int i = 1;; i++) {
                        ...
                        final BigDecimal corr = xpowi.divide(new BigDecimal(
                                ifac), mcTay);
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            if (res.compareTo(p) > 0) {
                ...
                if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
                     ...
                     */
                    final double xUlpDbl = 0.5 * res.ulp().doubleValue()
                            * res.doubleValue();
                     ...
                     */
                    final int k = (int) (Math.log(xUlpDbl) / Math.log(res
                            .doubleValue())) / 2;
                    final MathContext mcTay = new MathContext(err2prec(1.,
                            xUlpDbl / k));
                    for (int i = 1;; i++) {
                        /*
    ...
    static public BigDecimal asin(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ONE) > 0
            ...
            for (int i = 1;; i++) {
                ...
                final BigDecimal c = divideRound(multiplyRound(xpowi, ifacN),
    ...
    static public BigDecimal atan(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            final BigDecimal newx = divideRound(
                    hypot(1, y).subtract(BigDecimal.ONE), y);
             ...
             * of the ulp.
             */
            final double eps = x.ulp().doubleValue()
    ...
    static public BigDecimal cosh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            if (x.doubleValue() > 1.5) {
                 ...
                 */
                final double xUlpDbl = 0.5 * x.ulp().doubleValue()
                        * x.doubleValue();
                 ...
                 */
                final int k = (int) (Math.log(xUlpDbl) / Math.log(x
                        .doubleValue())) / 2;
                ...
                for (int i = 1;; i++) {
                    ...
                    final BigDecimal corr = xpowi.divide(new BigDecimal(ifac),
                            mcTay);
                    resul = resul.add(corr);
                    if (corr.abs().doubleValue() < 0.5 * xUlpDbl) {
                        break;
    ...
    static public BigDecimal sinh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            if (x.doubleValue() > 2.4) {
                ...
                final BigDecimal resul = sinh(xhalf).multiply(cosh(xhalf))
                        .multiply(two);
                /*
                ...
                final MathContext mc = new MathContext(err2prec(0.5
                 ...
                 */
                final int k = (int) (x.precision() / Math.log10(1.0 / xhighpr
                        .doubleValue())) / 2;
                final MathContext mcTay = new MathContext(err2prec(
                        x.doubleValue(), xUlpDbl / k));
                for (int i = 1;; i++) {
                    /*
                     * TBD: at which precision will 2*i or 2*i+1 overflow?
                     */
    ...
    static public BigDecimal asinh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) == 0) {
            ...
            final double eps = 0.5 * x.ulp().doubleValue()
                    / Math.hypot(1., xDbl);
            final MathContext mc = new MathContext(err2prec(logx.doubleValue(),
                    eps));
    ...
    static public BigDecimal acosh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ONE) < 0) {
             ...
             */
            final BigDecimal logx = log(sqrt(
                    xhighpr.pow(2).subtract(BigDecimal.ONE)).add(xhighpr));
    ...
    static public BigDecimal gamma(final BigDecimal x) {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            if (x.compareTo(BigDecimal.ONE) != 0) {
                ...
                final BigDecimal gammCompl = BigDecimal.ONE
                        .subtract(gamma(mcloc));
                ...
                for (int n = 2;; n++) {
                    ...
                    /* zeta(n) -1 */
                    final BigDecimal zetm1 = zeta(n, m)
    ...
    static public BigDecimal gamma(final Rational q, final MathContext mc) {
        if (q.isBigInteger()) {
            ...
            final double deltx = 5. * Math.pow(10., -mc.getPrecision())
                    / psi(qdbl);
     ...
     * @since 2009-06-01
     */
    static public BigDecimal mod2pi(final BigDecimal x) {
        /*
     ...
     * @since 2009-07-31
     */
    static public BigDecimal modpi(final BigDecimal x) {
        /*
    ...
    static public BigDecimal zeta(final int n, final MathContext mc) {
        ...
        if (n % 2 == 0) {
             ...
             */
            final MathContext mcpi = new MathContext(mc.getPrecision()
             ...
             */
            final MathContext mcloc = new MathContext(2 + mc.getPrecision()
                    + (int) Math.log10(n));
    ...
    static protected BigDecimal broadhurstBBP(final int n, final int p,
            final int a[], final MathContext mc) {
        /*
        ...
        for (int c = 0;; c++) {
            ...
            final MathContext mcloc = new MathContext(1 + err2prec(
                    r.doubleValue(), eps));
    ...
    static public BigDecimal addRound(final BigDecimal x, final BigDecimal y) {
         ...
         * |err(y)|+|err(x)|
         */
        final double errR = Math.abs(y.ulp().doubleValue() / 2.)
                + Math.abs(x.ulp().doubleValue() / 2.);
        final MathContext mc = new MathContext(err2prec(resul.doubleValue(),
                errR));
        return resul.round(mc);
    } /* addRound */
    ...
    static public BigDecimal multiplyRound(final BigDecimal x,
         ...
         */
        final MathContext mc = new MathContext(Math.min(x.precision(),
                y.precision()));
        return resul.round(mc);
    } /* multiplyRound */
    ...
    static public BigComplex multiplyRound(final BigComplex x,
            final BigComplex y) {
        final BigDecimal R = subtractRound(multiplyRound(x.re, y.re),
                multiplyRound(x.im, y.im));
        final BigDecimal I = addRound(multiplyRound(x.re, y.im),
                multiplyRound(x.im, y.re));
        return new BigComplex(R, I);
    } /* multiplyRound */
    ...
    static public BigComplex invertRound(final BigComplex z) {
        if (z.im.compareTo(BigDecimal.ZERO) == 0) {
             ...
             */
            BigDecimal r = addRound(z.re,
                    divideRound(multiplyRound(z.im, z.im), z.re));
            BigDecimal i = addRound(z.im,
                    divideRound(multiplyRound(z.re, z.re), z.im));
