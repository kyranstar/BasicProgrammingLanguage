...
public class BigIntegerPoly implements Cloneable {
     ...
     *            the zero-based index of the coefficient. n=0 for the constant
     *            term. If the polynomial has not yet the degree to need this
     *            coefficient, the intermediate coefficients are implicitly set
     *            to zero.
     * @param value
     *            the new value of the coefficient.
     */
    ...
    public BigIntegerPoly[] divideAndRemainder(final BigIntegerPoly val) {
         ...
         */
        if (valSimpl.degree() == 0
                && valSimpl.a.firstElement().compareTo(BigInteger.ZERO) == 0) {
        ...
        if (thisSimpl.degree() < valSimpl.degree()) {
            ...
            /*
             * long division. Highest degree by dividing the highest degree of
             * this thru val. At this point an exception is thrown if the
             * polynomial division cannot be done with integer coefficients.
             */
            ret[0] = new BigIntegerPoly();
            final BigInteger[] newc = thisSimpl.a.lastElement()
                    .divideAndRemainder(valSimpl.a.lastElement());
            if (newc[1].compareTo(BigInteger.ZERO) != 0) {
                throw new ArithmeticException("Incompatible leading term in "
                        + this + " / " + val);
            }
            ret[0].set(thisSimpl.degree() - valSimpl.degree(), newc[0]);
            
            /*
             * recurrences: build this - val*(1-termresult) and feed this into
             * another round of division. Have intermediate ret[0]+ret[1]/val.
             */
    ...
    public BigIntegerPoly derive() {
        if (a.size() <= 1) {
            ...
            for (int i = 1; i <= degree(); i++) {
                final BigInteger c = a.elementAt(i).multiply(
                        new BigInteger("" + i));
                d.set(i - 1, c);
            }
            return d;
    ...
    public BigIntegerPoly binomialTInv(final int maxdeg) {
        ...
        for (int i = 0; i <= maxdeg; i++) {
            ...
            for (int j = 0; j <= i && j < a.size(); j++) {
                if ((j + i) % 2 != 0) {
                    c = c.subtract(a.elementAt(j).multiply(
                            BigIntegerMath.binomial(i, j)));
                } else {
                    c = c.add(a.elementAt(j).multiply(
                            BigIntegerMath.binomial(i, j)));
                }
            }
            r.set(i, c);
    ...
    public Vector<BigInteger> iroots() {
        ...
        final Vector<BigInteger> res = new Vector<BigInteger>();
        ...
        if (a.elementAt(l).compareTo(BigInteger.ZERO) != 0) {
            final Vector<BigInteger> cand = BigIntegerMath.divisors(a
                    .elementAt(l).abs());
    ...
    protected Vector<BigIntegerPoly> i2roots() {
         ...
         */
        final Vector<BigIntegerPoly> res = new Vector<BigIntegerPoly>();
         ...
         * scaled by the expected division with values of b (which are all <=
         * a.firstele). Number of decimal digits in bsco by using a log2->log10
         * rough estimate and adding 6 safety digits
         */
        final RatPoly thisDec = toRatPoly();
        final Vector<BigComplex> roo = thisDec.roots(6 + (int) (0.3 * bsco
                .bitCount()));
         ...
         * a, b and c where b is restricted to a signed divisor of the constant
         * coefficient. Solve z*(c*z+a)=-b or c*z+a = -b/z or -b/z-c*z = some
         * integer a.
         */
        for (final BigComplex z : roo) {
            for (final BigInteger bco : b) {
                for (final BigInteger cco : c) {
                     ...
                     * this case is already covered by using iroots().
                     */
                    if (bco.signum() != 0) {
                        for (int sig = -1; sig <= 1; sig += 2) {
                            final BigInteger bcosig = sig > 0 ? bco : bco
                                    .negate();
                            /*
                             * -a = b/z+c*z has real part b*Re(z)/|z|^2+c*Re(z)
                             * = Re z *( b/|z|^2+c)
                             */
                            BigDecimal negA = BigDecimalMath.add(BigDecimalMath
                                    .divideRound(bcosig, z.norm()), cco);
                            negA = negA.multiply(z.re);
                            /*
                             * convert to a with round-to-nearest
                             */
                            final BigInteger a = negA.negate().add(half)
                                    .toBigInteger();
                             ...
                             * term to the results.
                             */
                            final BigIntegerPoly dtst = new BigIntegerPoly(""
                                    + bcosig + "," + a + "," + cco);
                            try {
    ...
    public Vector<BigIntegerPoly> ifactor() {
         ...
         */
        final Vector<BigIntegerPoly> fac = new Vector<BigIntegerPoly>();
        ...
        for (final BigIntegerPoly i : pol2) {
            /*
             * the internal loop catches cases with higher powers of individual
             * polynomials (of actual degree 2 or 4...)
             */
         ...
         */
        if (res[0].degree() > 0
                || res[0].a.firstElement().compareTo(BigInteger.ONE) != 0) {
            fac.add(res[0]);
        }
        return fac;
    } /* ifactor */
